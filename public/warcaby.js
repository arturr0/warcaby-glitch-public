  ...
  if (pawnSelected) {
    ...
    for (let j = 0; j < Board.length; j++) {
      if (Board[j].isBlack && X > Board[j].rectCenter - 32 && X < Board[j].rectCenter + 32 &&
        ...
        // check = true;
        current = pawnPlayed;
        socket.emit('state', Board, Pawns, Greenturn, check, current, killConditions, room); // Send the move to the server
        socket.emit('move', { x: targetPos.x, y: targetPos.y, oldX: movingPawnOldPos.x, oldY: movingPawnOldPos.y }, room); // Send the move to the server
        pawnSelected = false;
        isPawnMoving = true;
        //Greenturn = !Greenturn;
      }
    ...
    for (let i = 0; i < killersOpt.length; i++) {
      ...
      if (((killersOpt[i][3] && !Greenturn && Player == 1) || (!killersOpt[i][3] && Greenturn  && Player == 2)) &&
          X > killersOpt[i][5] - 32 && X < killersOpt[i][5] + 32 && Y > killersOpt[i][6] - 32 && Y < killersOpt[i][6] + 32) {
            ...
            killOpt();
            stepKill();
            socket.emit('killer mode', killersOptMode, Pawns, room);
        }
...
function stepKill() {
  ...
  for (let i = 0; i < killConditions.length; i++)
    ...
    socket.emit('state', Board, Pawns, Greenturn, check, current, killConditions, room);
    socket.emit('move', { x: targetPos.x, y: targetPos.y, oldX: movingPawnOldPos.x, oldY: movingPawnOldPos.y }, room);
    break;
  }
  //killConditions = [];
